...
form=document.querySelector('.lorem-form')
amount=document.getElementById('amount')
article=document.querySelector('.lorem-text')
// Function
function showAmount(e){
  ...
  let sliceArray;
  if(value>0 && value<=9){
      sliceArray=text.slice(0,value)
      mapArray=sliceArray.map(function(item){
        return `<p class="result">${item}</p>`
      }).join("")
  article.innerHTML=mapArray
  }
  if(value<0 || value>9 || value===NaN){
    const random=Math.floor(Math.random() * 9)
    sliceArray=text[random]
    article.innerHTML=sliceArray
  }
}
// Add Event Listenner
addEventListener('submit',showAmount)